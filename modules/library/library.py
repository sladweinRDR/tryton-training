...
class Author(ModelSQL, ModelView):
    ...
    def getter_latest_book(cls, authors, name):
        ...
        sub_query = sub_book.select(
            sub_book.author,
            Max(
                Coalesce(sub_book.publishing_date, datetime.date.min),
                window=Window([sub_book.author]),
            ).as_("max_date"),
...
class Book(ModelSQL, ModelView):
    ...
    exemplaries = fields.One2Many(
        "library.book.exemplary", "book", "Exemplaries"
    )
    ...
    isbn = fields.Char(
        "ISBN", size=13, help="The International Standard Book Number"
    )
    ...
    page_count = fields.Integer(
        "Page Count", help="The number of page in the book"
    )
    ...
    number_of_exemplaries = fields.Function(
        fields.Integer("Number of exemplaries"), "getter_number_of_exemplaries"
    )
...
class Exemplary(ModelSQL, ModelView):
    ...
    book = fields.Many2One(
        "library.book", "Book", ondelete="CASCADE", required=True
    )
