...
class Genre(ModelSQL, ModelView):
    ...
    editors = fields.Many2Many(
        "library.editor-library.genre", "genre", "editor", "Editors", readonly=True
    )
...
class Author(ModelSQL, ModelView):
    ...
    latest_book = fields.Function(
        fields.Many2One(
            "library.book", "Latest Book", states={"invisible": ~Eval("books", False)}
        ),
        "getter_latest_book",
    )
...
class Exemplary(ModelSQL, ModelView):
    ...
    acquisition_price = fields.Numeric(
        "Acquisition Price",
        digits=(16, 2),
        domain=["OR", ("acquisition_price", "=", None), ("acquisition_price", ">", 0)],
    )
