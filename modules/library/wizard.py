...
class CreateExemplaries(Wizard):
    ...
    parameters = StateView(
        ...
        [
            ...
            Button(
                "Create", "create_exemplaries", "tryton-go-next", default=True
            ),
        ],
    )
...
class CreateExemplariesParameters(ModelView):
    ...
    exemplaries = fields.Many2Many(
        "library.book.exemplary", None, None, "Exemplaries"
    )
...
class FuseBooks(Wizard):
    ...
    select_main = StateView(
        "library.book.fuse.select_main",
    ...
    def default_select_main(self, name):
        ...
        return {
            ...
            "number_of_exemplaries": sum(
                x.number_of_exemplaries for x in books
            ),
        }
    ...
    def transition_check_compatibility(self):
        ...
        if bad_matches:
            self.raise_user_warning(
                "bad_matches_warning" + str(self.select_main.main_book.id),
                "bad_matches",
    ...
    def transition_merge(self):
        ...
        Exemplary.write(
            sum([list(x.exemplaries) for x in other_books], []),
            {"book": book.id},
        )
...
class FuseBooksSelectMain(ModelView):
    ...
    main_book = fields.Many2One(
        "library.book",
        "Main Book",
        required=True,
        domain=[("id", "in", Eval("selected_books"))],
    )
    selected_books = fields.Many2Many(
        "library.book", None, None, "Selected books", readonly=True
    )
    number_of_exemplaries = fields.Integer(
        "Number of exemplaries", readonly=True
    )
...
class FuseBooksPreview(ModelView):
    ...
    final_book = fields.One2Many(
        "library.book", None, "Final book", readonly=True
    )
    number_of_exemplaries = fields.Integer(
        "Number of exemplaries", readonly=True
    )
