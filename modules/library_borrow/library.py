...
class User(ModelSQL, ModelView):
    ...
    late_checkedout_books = fields.Function(
        fields.Integer(
            "Late checked-out books",
            help="The number of books a user is late returning",
        ),
        "getter_checkedout_books",
    )
    expected_return_date = fields.Function(
        fields.Date(
            ...
            help="The date at which the user is (or was) expected to return his books",
        ),
        "getter_checkedout_books",
        searcher="searcher_expected_return_date",
    )
    ...
    def search_expected_return_date(cls, name, clause):
        ...
        query_table = user.join(
            checkout, "LEFT OUTER", condition=checkout.user == user.id
        )
...
class Checkout(ModelSQL, ModelView):
    ...
    user = fields.Many2One(
        "library.user", "User", required=True, ondelete="CASCADE", select=True
    )
    exemplary = fields.Many2One(
    ...
    expected_return_date = fields.Function(
        fields.Date(
            "Expected retrun date",
            help="The date at which the exemplary is supposed to be returned",
        ),
...
class Book(metaclass=PoolMeta):
    ...
    is_available = fields.Function(
        fields.Boolean(
            "Is available",
            help="If True, at least an exemplary of this book is currently available for borrowing",
        ),
        "getter_is_available",
        searcher="searcher_is_available",
    )
...
class Exemplary(metaclass=PoolMeta):
    ...
    is_available = fields.Function(
        fields.Boolean(
            "Is available",
            help="If True, the exemplary is currently available for borrowing",
        ),
        "getter_is_available",
        searcher="searcher_is_available",
    )
