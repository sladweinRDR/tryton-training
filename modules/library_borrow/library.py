...
class User(ModelSQL, ModelView):
    ...
    checkedout_books = fields.Function(
        fields.Integer('Checked-out books', help='The number of books has currently checked out'),
        'getter_checkedout_books')
    late_checkedout_books = fields.Function(
        fields.Integer('Late checked-out books', help='The number of books a user is late returning'),
        'getter_checkedout_books')
    expected_return_date = fields.Function(
        fields.Date('Expected return date', help="The date at which the user is (or was) expected to return his books"),
        "getter_checkedout_books", searcher='searcher_expected_return_date')
    ...
    def getter_checkedout_books(cls, users, name):
        ...
        cursor.execute(*checkout.select(checkout.user, column,
                where=where & checkout.user._in([x.id for x in users]),
...
class Checkout(ModelSQL, ModelView):
    ...
    return_date = fields.Date('Return Date', domain=[
        ...
        depends=['date'])
    expected_return_date = fields.Function(
        fields.Date('Expected retrun date', help="The date at which the exemplary is supposed to be returned"),
...
class Book(metaclass=PoolMeta):
    ...
    is_available = fields.Function(
        fields.Boolean('Is available', help="If True, at least an exemplary of this book is currently available for borrowing"),
        'getter_is_available', searcher='searcher_is_available')
