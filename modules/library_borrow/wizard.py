...
class Borrow(Wizard):
    ...
    select_books = StateView(
        "library.user.borrow.select_books",
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update(
            {
                "unavailable": "Exemplary %(exemplary)s is unavailable for checkout",
            }
        )
    ...
    def default_select_books(self, name):
        ...
        if Transaction().context.get("active_model") == "library.user":
            ...
            books = (
                Pool()
                .get("library_book")
                .browse(Transaction().context.get("active_ids"))
            )
            for book in books:
...
class BorrowSelectBooks(ModelView):
    ...
    checkouts = fields.Many2Many(
        "library.user.checkout", None, None, "Checkouts", readonly=True
    )
...
class Return(Wizard):
    ...
    select_checkouts = StateView(
        "library.user.return.checkouts",
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update(
            {
                "multiple_users": "You can't return checkouts from different users at once",
                "available": "Can't return an available exemplary",
            }
        )
...
class ReturnSelectCheckouts(ModelView):
    ...
    checkouts = fields.Many2Many(
        ...
        None,
        "Checkouts",
        domain=[("user", "=", Eval("user")), ("return_date", "=", None)],
    )
    date = fields.Date("Date", required=True, domain=[("date", "<=", Date())])
